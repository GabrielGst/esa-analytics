...
type props = {
  ...
  activitySelection: rowSelection
}

export default function GenPptxTracking({
  activityData,
  activitySelection
}: props) {
  ...
  function handleClick() {
    ...
    slideTitle.addText(
      "Automatic Extraction of Annual Review App",
    ...
    function addTableSlide( arr: Activity | Story ){
      ...
      if (arr.type === 'activity') { // 
        ...
        let arrTabRows1 = [
          [
            { text: "(MP) Technical Officer:", options: { bold: true, align: "right" } },
            { text: arr.technicalOfficer, options: { bold: true, align: "right" } },
            // { text: "Co: " + "XXX", options: { bold: true, align: "right" } },
            { text: "(MP) End TRL: " + arr.trls, options: { align: "left" } },
            { text: "(MP) Prog. Ref: " + arr.Title, options: { align: "left" } },
            { text: arr.ssapId, options: { align: "left" } }, // to update with Prog Ref
          ],
          [
            { text: "(MP) Supplier Name:", options: { colspan: 1, bold: true, align: "right" } },
            { text: arr.supplierName, options: { colspan: 3, bold: true, align: "right" } },
            { text: "(MP) Supplier Code\n" + arr.supplierCode, options: { bold: true, align: "right" } },
            { text: "(MP) Fundcode: " + arr.fundCode + "\nContract Number: " + arr.contractNumber, options: { bold: true, align: "right", colspan: 1 } },
          ],
          // [
          //   { text: "Description:", options: { bold: true, align: "right" } },
          //   { text: arr.description , options: { colspan: 5, align: "left" } }, //"€ " + parseFloat(arr.field_9).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })
          // ],
          // [
          ...
          // ],
          [
            { text: "(ARPP) Start of Activity:\n" + arr.arap_StartofActivity, options: { colspan: 1, bold: true, align: "right" } },
            { text: "(ARPP) Expected End of Activity:\n" + arr.arap_EED , options: { colspan: 1, bold: true, align: "right" } },
            { text: "(ARPP) Planned start TRL: " + arr.arap_PlannedStartTRL , options: { colspan: 1, align: "left" } },
            { text: "(ARPP) Planned end TRL: " + arr.arap_PlannedEndTRL , options: { colspan: 1, align: "left" } },
            { text: "(ARPP) Overall Amount: " + formatMoney(parseFloat(arr.arap_OverallAmount), { symbol: "€", precision: 2, thousand: ".", decimal: "," }) , options: { colspan: 2, align: "left" } },
          ],
          [
            { text: "(ARPP) Description:", options: { bold: true, align: "right" } },
            { text: arr.arap_Description , options: { colspan: 7, align: "left" } },
          ],
          [
            { text: "(ARPP) Performance of Company:", options: { bold: true, align: "right" } },
            { text: arr.arap_PerformanceofCompagny, options: { colspan: 7, align: "left" } },
          ],
                    [
            { text: "(ARPP) Prospect for use:", options: { bold: true, align: "right" } },
            { text: arr.arap_ProspectforUse, options: { colspan: 7, align: "left" } },
          ],
          [
            { text: "(ARPP) Note:", options: { bold: true, align: "right" } },
    ...
    const filteredArray = activityData.filter((item) => Object.keys(activitySelection).some(el => el === item.ssapId))
